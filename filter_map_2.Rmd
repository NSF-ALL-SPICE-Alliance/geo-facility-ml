...
```{r}
geocoded_facilities_filtered[, service_code_info := sapply(
  strsplit(service_code_info, " * "), 
  function(abbrev_list) {
    paste(full_definitions[abbrev_list], collapse = " * ")
  }
)]
```
...
for (category in names(abbrev_dict)) {
  abbrev_dict[[category]] <- full_definitions[abbrev_dict[[category]]]
}
```
...
server <- function(input, output, session) {
  ...
  observeEvent(input$overall_category, {
    ...
    # Get full definitions based on selected overall category
    choices <- full_definitions[abbrev_dict[[input$overall_category]]]
  ...
  filtered_data <- reactive({
    ...
    filter_data <- geocoded_facilities_filtered[grep(input$service, service_code_info)]
    return(filter_data)
  })
