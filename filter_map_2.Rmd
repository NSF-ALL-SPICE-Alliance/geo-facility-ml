...
```{r}
states <- c("District of Columbia", "New Jersey", "Rhode Island", "Puerto Rico", "Massachusetts", "Guam",
            "Connecticut", "U.S. Virgin Islands", "Maryland", "American Samoa", "Delaware", "Florida",
            "New York", "Pennsylvania", "Ohio", "Northern Mariana Islands", "California", "Illinois",
            "Hawaii", "North Carolina", "Virginia", "Georgia", "Indiana", "South Carolina", "Michigan",
            "Tennessee", "New Hampshire", "Washington", "Texas", "Kentucky", "Wisconsin", "Louisiana",
            "Alabama", "Missouri", "West Virginia", "Minnesota", "Vermont", "Arizona", "Mississippi",
            "Oklahoma", "Arkansas", "Iowa", "Colorado", "Maine", "Oregon", "Utah", "Kansas", "Nevada",
            "Nebraska", "Idaho", "New Mexico", "South Dakota", "North Dakota", "Montana", "Wyoming", "Alaska")

pop_density <- c(11131, 1263, 1060, 936, 898, 824, 747, 737, 637, 578, 529, 422, 415, 290, 288, 274, 250, 226,
                 223, 223, 221, 192, 192, 179, 178, 173, 157, 118, 117, 115, 109, 106, 101, 90, 74, 72, 70, 65,
                 63, 59, 59, 57, 57, 45, 44, 42, 36, 29, 26, 24, 17, 12, 11, 7.8, 6.0, 1.3)


population_density <- data.frame(State = states, Population_Density = pop_density)
```


```{r}
mapbox_test_data <- geocoded_facilities_filtered[grepl("Mental health treatment", service_code_info)]
mapbox_test_data[, service_code_info := "Mental health treatment"]
```
...
Sys.setenv(MAPBOX_TOKEN = "pk.eyJ1IjoicXVpbm5kYW5pZWwxMSIsImEiOiJjbHl5cnlmcHgwM2hxMmtwcWFuZXg1b2J5In0.PAKI_uHcXXDelkBcsY9L9g")

# hosting

# Add markers to the map
mapbox_test_data %>%
  dplyr::mutate(color = "cyan") %>%
...
DATA manip
```{r}
mental_health_treatment_layer <- mapbox_test_data

substance_use_treatment_layer <- geocoded_facilities_filtered[grepl("Substance use treatment", service_code_info)]
substance_use_treatment_layer[, service_code_info := "Substance use treatment"]

cocurring_substance_use_layer <- geocoded_facilities_filtered[grepl("Treatment for co-occurring substance use plus either serious mental health illness in adults/serious emotional disturbance in children", service_code_info)]
cocurring_substance_use_layer[, service_code_info := "Treatment for co-occurring substance use plus either serious mental health illness in adults/serious emotional disturbance in children"]

outpatient_layer <- geocoded_facilities_filtered[grepl("Outpatient", service_code_info)]
outpatient_layer[, service_code_info := "Outpatient"]

outpatient_mh_facility_layer <- geocoded_facilities_filtered[grepl("Outpatient mental health facility", service_code_info)]
outpatient_mh_facility_layer[, service_code_info := "Outpatient mental health facility"]
...
MAP settings
```{r}
data_separated <- geocoded_facilities_filtered %>%
  separate_rows(service_code_info, sep = " \\* ")

drop_na(data_separated)

data_sf <- st_as_sf(data_separated, coords = c("longitude", "latitude"), crs = 4326)

data_split <- split(data_sf, data_sf$service_code_info)


for(sector in names(data_split)) {
  map <- map + mapview(data_split[[sector]], zcol = "service_code_info", layer.name = sector)
}

mapview(states_sf) + mapview(data_sf, color = "red", size = 0.1, popup = "name1")
```
