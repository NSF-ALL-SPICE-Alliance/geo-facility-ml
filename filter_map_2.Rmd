...
MAP settings
```{r}
facilities_sf <- st_as_sf(facilities_long, coords = c("longitude", "latitude"), crs = 4326)
services <- unique(facilities_long$service)
```
...
for (service in services) {
  service_sf <- facilities_sf %>% filter(service == !!service)
  layers[[service]] <- mapview(service_sf, layer.name = service, legend = FALSE)
}

# Combine all layers into one map
combined_map <- mapview(counties_sf, layer.name = "Counties")

for (service in services) {
  combined_map <- combined_map + layers[[service]]
}
...
m <- leaflet() %>%
  addTiles() %>%
  addPolygons(data = counties_sf, color = "black", weight = 1, smoothFactor = 0.5, 
              opacity = 1.0, fillOpacity = 0.5, group = "Counties")

# Get unique services
services <- unique(facilities_long$service)

# Add a layer for each service
for (service in services) {
  service_sf <- facilities_sf %>% filter(service == !!service)
  m <- m %>%
    addCircleMarkers(data = service_sf, ~longitude, ~latitude, color = "red", radius = 5,
                     group = service, label = ~facility_name)
}

# Add layer control with radio buttons (base groups)
m <- m %>%
  addLayersControl(
    baseGroups = services,
