...
server <- function(input, output, session) {
  ...
  observe({
    ...
    leafletProxy("map", data = data) %>%
     ...
     addHeatmap(
    lng = facilities_long$longitude, 
    lat = facilities_long$latitude,
...
map <- leaflet() %>%
  addTiles() %>%
  setView(lng = -73.9165, lat = 40.7114, zoom = 2)
...
map <- map %>%
  addHeatmap(
    ...
    lat = facilities_long$latitude,
    intensity = rep(1, nrow(facilities_long)), 
    blur = 15, max = 0.05, radius = 15,
    group = "Heatmap"
  )

# Add a layer for each unique service
services <- unique(facilities_long$service)

# Add circle markers for each service
for (service in services) {
  if (!is.na(service) && service != "NA") {
    filtered_data <- facilities_long %>% filter(service == !!service)
...
# Add layer control (start with all layers visible for testing)
map <- map %>%
  addLayersControl(
    baseGroups = c("Heatmap", "Facilities"),
    overlayGroups = services,
